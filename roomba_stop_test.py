...
def log_and_print(trial, speed, time, odom_change, camera_change, current_battery, stop_yaw):
...
with open ('Roomba_Stop_Test.csv', 'ab') as csvfile:
	...
	fieldnames = ['trial', 'speed', 'time', 'odom_change', 'camera_change', 'current_battery', 'stopping distance']
	...
	for trial in range(0, 50):
		stop_yaw = runFor(3, velocity)
		sleep(1.5)
		set_transform("odomFrame", "odometry_final", lookup_odom())
		set_transform("camFrame", "camera_final", check_camera())
		log_and_print(trial, velocity, time, get_transform("odometry_initial", "odometry_final"), get_transform("camera_initial", "camera_final"), lookup_battery(), stop_yaw)
